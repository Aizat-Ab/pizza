...
const Sort = () => {    
    const soirtItem = ['популярности', 'цене', 'алфавиту'];
    ...
    return (
        <div ref={sortRef} className="sort">
              <div className="sort__label">
                ...
                <b>Сортировка по:</b>
                <span onClick={()=>setSortPopup(true)}>{soirtItem[sortItem]}</span>
              </div>
              {sortPopup && <div className="sort__popup">
                <ul>
                  {soirtItem.map((item, index)=>{
                      return <li onClick={()=>onClick(index)} 
                      className = {index === sortItem ? 'active' : ''}>{item}</li>
                  })}
                </ul>
              </div>}
            </div>
